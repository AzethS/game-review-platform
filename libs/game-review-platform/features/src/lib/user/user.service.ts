...
import { ApiResponse, IGame, IUser } from '@game-platform/shared/api';
...
export class UserService {
  ...
  getUserById(id: string): Observable<IUser> {
    return this.http.get<ApiResponse<IUser>>(`${this.apiUrl}/user/${id}`).pipe(
      map(res => res.results!)
    );
  }
  ...
  public create(userData: any, options?: any): Observable<IUser> {
    return this.http
      .post<IUser>(this.apiUrl + this.endpoint, userData, {
        ...options,
        ...httpOptions,
      })
      .pipe(
  ...
  public update(userId: string, userData: any, options?: any): Observable<any> {
    return this.http
      .put<IUser>(`${this.apiUrl}${this.endpoint}/${userId}`, userData, {
        ...options,
        ...httpOptions,
      })
      .pipe(
  ...
  getMembers(options?: any): Observable<IUser[] | null> {
    return this.http
      .get<ApiResponse<IUser[]>>(`${this.apiUrl}${this.endpoint}/members`, {
        ...options,
        ...httpOptions,
      })
      .pipe(
  ...
  addOwnedGame(userId: string, gameId: string) {
    return this.http.patch(`${this.apiUrl}/user/${userId}/owned-games`, {
      gameId,
    });
  }

  getRecommendGamesForUser(userId: string): Observable<IGame[]> {
    return this.http.get<{ recommendations: IGame[] }>(
      `http://localhost:3100/api/neo4j/recommend/user/${userId}`
